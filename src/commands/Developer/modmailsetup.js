...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        switch (sub) {
            ...
            if (!interaction.guild) return await interaction.reply({ content: `You **cannot** use this command in **DMs**!`, flags: [MessageFlags.Ephemeral] })
 
            const data1 = await modschema.findOne({ Guild: interaction.guild.id });
            if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) return await interaction.reply({ content: 'You **do not** have the permission to do that!', flags: [MessageFlags.Ephemeral] });
 
            if (data1) return await interaction.reply({ content: `You have **already** set up the **modmail** in this server. \n> Do **/modmail disable** to undo.`, flags: [MessageFlags.Ephemeral] });
            else {
            ...
            if (!data) return await interaction.reply({ content: `You have **not** set up the **modmail** in this server.`, flags: [MessageFlags.Ephemeral]  });
            else {
            ...
            if (!usedata) return await interaction.reply({ content: `You **do not** have an open **modmail**!`, flags: [MessageFlags.Ephemeral] });
            else {
 
                const channel = await client.channels.cache.get(usedata.Channel);
                if (!channel) {
 
                    await interaction.reply({ content: `Your **modmail** has been **closed**!`, flags: [MessageFlags.Ephemeral] });
                    await moduses.deleteMany({ User: interaction.user.id });
 
                } else {
 
                    await interaction.reply({ content: `Your **modmail** has been **closed** in **${channel.guild.name}**!`, flags: [MessageFlags.Ephemeral] });
                    await moduses.deleteMany({ User: interaction.user.id });
